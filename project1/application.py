...
def login():
    ...
    if request.method == "POST":
        ...
        if db.execute("SELECT id FROM users1 WHERE email= :email", {"email": email}).fetchone() is not None:
            return render_template("login.html", work="Login",
                                   error_message="The user has already registered. Please Login.")
        password = request.form.get("Password")
        db.execute("INSERT INTO users1 (email, password) VALUES (:email, :password)",
...
def search():
    ...
    if request.method == "POST":
        ...
        email = request.form.get("Email")
        user = db.execute("SELECT id, password FROM users1 WHERE email= :email", {"email": email}).fetchone()
        if user is None:
...
def booklist():
    ...
    if book_column == "year":
        book_list = db.execute("SELECT * FROM books2 WHERE year = :query", {"query": query}).fetchall()
    else:
        book_list = db.execute("SELECT * FROM books2 WHERE UPPER(" + book_column + ") = :query ORDER BY title",
    ...
    if len(book_list):
        ...
        book_list = db.execute("SELECT * FROM books2 WHERE UPPER(" + book_column + ") LIKE :query ORDER BY title",
                               {"query": "%" + query.upper() + "%"}).fetchall()
...
def detail(book_id):
    ...
    book = db.execute("SELECT * FROM books2 WHERE id = :book_id", {"book_id": book_id}).fetchone()
    if book is None:
    ...
    res = requests.get("https://www.goodreads.com/book/review_counts.json",
                       params={"key": "OZtotUcmFicJ8DMifZvRQw", "isbns": book.isbn}).json()["books"][0]
    ...
    for review in reviews:
        email = db.execute("SELECT email FROM users1 WHERE id = :user_id", {"user_id": review.user_id}).fetchone().email
...
def api(ISBN):
    book = db.execute("SELECT * FROM books2 WHERE isbn = :ISBN", {"ISBN": ISBN}).fetchone()
    if book is None:
